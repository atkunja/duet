pub fn architect(task: &str, overview: &str) -> String {
    format!(
        r#"You are the architecture agent in Duet. Inspect the repository but DO NOT edit any file.
...
pub fn implementer(task: &str, architecture: &str, test_command: &str) -> String {
    format!(
        r#"You are the implementation agent in Duet. Work directly in the current isolated Git worktree.
...
pub fn reviewer(task: &str, architecture: &str, diff: &str, verification: &str) -> String {
    format!(
        r#"You are the adversarial reviewer in Duet. Do not edit files. Find real defects, not style trivia.
...
pub fn repair(
    ...
    format!(
        r#"You are the repair agent in Duet, repair round {round}. Work in the current isolated worktree.
