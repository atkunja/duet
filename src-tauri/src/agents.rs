use crate::{
    models::{ArchitecturePlan, ReviewResult},
...
impl Agent for CodexAgent {
    ...
    async fn execute(
        ...
        let sandbox = if matches!(request.role, AgentRole::Architect | AgentRole::Reviewer) {
            "read-only"
        } else {
            "workspace-write"
        };
...
pub fn parse_architecture(text: &str) -> Result<ArchitecturePlan> {
    parse_json_object(text).ok_or_else(|| anyhow!("Codex did not return a valid architecture plan"))
}
pub fn parse_review(text: &str) -> Result<ReviewResult> {
    parse_json_object(text).ok_or_else(|| anyhow!("Codex did not return a valid review"))
}
...
mod tests {
    ...
    fn extracts_json_from_markdown_noise() {
        let result: ReviewResult = parse_json_object(
    ...
    #[test]
    fn preserves_snake_case_architecture_arrays() {
        let plan=parse_architecture(r#"{"goal":"g","summary":"s","files_to_modify":["src/a.rs"],"files_to_add":["src/b.rs"],"implementation_steps":["edit"],"risks":["race"],"tests_required":["test"]}"#).unwrap();
        assert_eq!(plan.files_to_modify, vec!["src/a.rs"]);
    ...
    #[test]
    fn survives_malformed_jsonl() {
        assert!(normalize_jsonl("not json\n{\"result\":\"yes\"}").contains("yes"));
    }
}
