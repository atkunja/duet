import { ArrowRight, ChevronDown, Code2, FlaskConical, GitBranch, PanelRight, Play, Settings2, ShieldCheck, Sparkles } from "lucide-react";
...
export function TaskComposer({project,onRun,onError,busy}:{project:Project;onRun:(request:StartRunRequest)=>void;onError?:(message:string)=>void;busy:boolean}){
  const [task,setTask]=useState(""); const [test,setTest]=useState(project.testCommand); const [benchmark,setBenchmark]=useState(project.benchmarkCommand); const [repairs,setRepairs]=useState(3); const [parallel,setParallel]=useState(false); const [mock,setMock]=useState(false); const [toolsOpen,setToolsOpen]=useState(false); const [agents,setAgents]=useState(defaultAgentModeSelection);
  const [codexModels,setCodexModels]=useState<ModelOption[]>();
  useEffect(()=>{let disposed=false;setTask("");setTest(project.testCommand);setBenchmark(project.benchmarkCommand);setParallel(false);api.getPreferences().then(preferences=>{if(!disposed)setRepairs(preferences.maxRepairs)}).catch(()=>{});return()=>{disposed=true}},[project.id]);
  ...
  return <main className="composer-page">
    ...
    <div className={`composer-wrap ${toolsOpen?"tools-open":""}`}>
      <div className="composer-main">
        ...
        <div className="composer-footer"><div className="composer-checks"><label className="check-row"><input type="checkbox" checked={mock} onChange={e=>setMock(e.target.checked)}/><FlaskConical size={15}/><span>Mock agents</span><small>No usage</small></label>{benchmark.trim()&&<label className="check-row"><input type="checkbox" checked={parallel} onChange={e=>setParallel(e.target.checked)}/><span>Parallel checks</span><small>Opt in only when safe</small></label>}</div><button className="primary-button" disabled={!task.trim()||!test.trim()||busy} onClick={submit}><Play size={15} fill="currentColor"/>{busy?"Starting…":`Run ${modeName(agents.mode)}`}<kbd>⌘ ↵</kbd></button></div>
        </div>
