...
export function TaskComposer({project,onRun,busy}:{project:Project;onRun:(request:StartRunRequest)=>void;busy:boolean}){
  ...
  useEffect(()=>{setTest(project.testCommand);setBenchmark(project.benchmarkCommand)},[project]);
  const submit=()=>{if(task.trim())onRun({projectId:project.id,task:task.trim(),testCommand:test,benchmarkCommand:benchmark||undefined,maxRepairs:repairs,mockAgents:mock})};
  return <main className="composer-page">
    ...
    <div className="composer-wrap">
      ...
      <div className="task-card">
        <textarea autoFocus value={task} onChange={e=>setTask(e.target.value)} placeholder="Describe a feature, bug fix, refactor, or performance goal…" onKeyDown={e=>{if((e.metaKey||e.ctrlKey)&&e.key==="Enter")submit()}}/>
        ...
        <div className="form-grid">
          <label><span>Test command</span><input value={test} onChange={e=>setTest(e.target.value)} placeholder="Auto-detected or custom command"/></label>
        ...
        <div className="composer-footer"><label className="check-row"><input type="checkbox" checked={mock} onChange={e=>setMock(e.target.checked)}/><FlaskConical size={15}/><span>Mock agents</span><small>No Claude/Codex usage</small></label><button className="primary-button" disabled={!task.trim()||busy} onClick={submit}><Play size={15} fill="currentColor"/>{busy?"Starting…":"Run Duet"}<kbd>⌘ ↵</kbd></button></div>
      </div>
      <div className="privacy-note"><ShieldCheck size={15}/><span>Source code, prompts, and outputs stay on this Mac. Duet uses only your authenticated local CLIs.</span></div>
    </div>
  </main>
}
